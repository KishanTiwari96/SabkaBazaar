...
export const Item = ({id} : {id:string}) => {
    ...
    return <div className="grid grid-cols-3">
        ...
        <div className="col-span-2 ml-5">
            <h3 className="ml-5 text-gray-500 text-lg font-medium ">{product.brand?.name || 'Unknown Brand'}</h3>
            <h3 className="ml-5 text-black text-xl font-medium ">{product.name}</h3>
            <div className="flex">
                
                <h3 className="ml-5 text-green-400 text-xl font-semibold ">₹{product.price}</h3>
            </div>
        </div>
